export class SceneDialogSpriteAnswerBox1 {
    ...
    open(plotspr, lines: Array<string>, gap = 80) {
        ...
        let itemw = scene.spr("vnt-answer-box1").get("display").width;
        let itemh = scene.spr("vnt-answer-box1").get("display").height;
        let boxh = scene.spr("vnt-dialog-box1").get("display").height;
        let left = (scene.game.get("display").width - itemw) / 2;
        let top = (scene.game.get("display").height - boxh - gap * lines.length) / 2;
        let chatmsg = scene.sprites["vnt-dialog-text1"];
        ...
        for (let line of lines) {
            idx++;
            let item = scene.getFreeSprite("vnt-answer-box1");
            let text = scene.getFreeSprite("vnt-answer-text1");
            ...
            text.active = true;
            if (idx == 1) {
                let cursor = scene.sprites["vnt-answer-cursor1"];
    ...
    close() {
        ...
        let pool = spr.scene.pools["vnt-answer-box1"];
        for (let item of pool) {
            item.active = false;
            if (item.custom) item.custom.flag = 0;
        }
        let pool2 = spr.scene.pools["vnt-answer-text1"];
        for (let item of pool2) item.active = false;
        let cursor = spr.scene.sprites["vnt-answer-cursor1"];
